...
function main() {
  ...
  if (cmd === 'add') {
    ...
    const metadata = parseMaybeJson(args[1]) || { source: 'cli' };
    if (!String(content).trim()) {
      console.error(JSON.stringify({ error: 'Missing content' }, null, 2));
      process.exit(1);
    }
    const row = addMemory(String(content).trim(), metadata);
  ...
  if (cmd === 'search') {
    ...
    console.log(JSON.stringify(searchMemory(String(query).trim(), limit), null, 2));
    return;
  }

  console.error('Usage:');
  console.error('  node index.cjs add "memory text"');
  console.error('  node index.cjs add "memory text" "{""topic"":""infra""}"');
  console.error('  node index.cjs search "query" [limit]');
  process.exit(1);
}
