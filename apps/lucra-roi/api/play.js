...
.grid{display:grid;grid-template-columns:1fr;gap:18px;margin-top:22px}@media(max-width:900px){.stamp{text-align:left}header{display:block}}
.bar{position:sticky;top:0;z-index:5;background:rgba(7,26,51,.96);backdrop-filter:blur(6px);border:1px solid var(--line);border-radius:12px;padding:8px 12px;margin-top:16px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px 14px}.bar .tile{border:0;background:none;padding:0;min-width:0}.bar .tile span{font-size:10px}.bar .tile strong{font-size:18px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bar .tile small{margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bar .err{grid-column:1/-1;margin:0}@media(max-width:700px){.bar{grid-template-columns:1fr 1fr;padding:6px 10px;gap:4px 10px}.bar .tile span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bar .tile strong{font-size:15px}.bar .tile small{display:none}}
...
.bars{display:flex;align-items:flex-end;gap:2px;height:90px;margin:8px 0 2px}body.size-compact .bars{height:64px}body.size-large .bars{height:150px}body.size-large .bar .tile strong{font-size:22px}body.size-large table{font-size:14px}
...
function render(){
  ...
  $('model').innerHTML='<header><div><div class="brand">LUCRA · REVENUE MODEL</div><h1>'+esc(f.dealName||'Your revenue model')+'</h1><div class="sub">Change your own numbers and watch the model move. The licence and the commercial terms are as proposed and cannot be changed here.</div></div><div class="stamp">'+(f.presenter?'Prepared by '+esc(f.presenter)+(f.presenterEmail?'<br>'+esc(f.presenterEmail):'')+'<br>':'')+(exp?'Link open until '+exp.toLocaleDateString():'')+'</div></header>'+
    ...
    '<div class="bar" id="bar" aria-live="polite"></div>'+
    '<div class="grid"><div class="panel" id="inputs"></div><div class="panel" id="results"></div></div>'+
...
function renderResults(){
  var o=OUT, f=FACTS, h='<div class="err" id="res-err" hidden></div>';
  // The four headline numbers live in a bar that stays on screen while they
  // change their inputs below it; averages across the term, updated as they type.
  if(o.errors&&o.errors.length){ $('bar').innerHTML='<div class="err">'+o.errors.map(esc).join('<br>')+'</div>'; $('results').innerHTML=h; $('term').innerHTML=''; $('term').hidden=true; return; }
  $('term').hidden=false;
  var payoff = o.free ? 'No licence to retire' : o.payoffMonth!==null ? 'Month '+(Math.round(o.payoffMonth*10)/10).toFixed(1) : money(o.balanceDue)+' left';
  var payoffNote = o.free ? 'Waived' : o.payoffMonth!==null ? money(o.licenceTotal)+' over the term' : 'at term end · '+money(o.licenceTotal)+' over the term';
  $('bar').innerHTML='<div class="tile"><span>Revenue generated / yr</span><strong>'+money(o.revenueYear)+'</strong><small>'+(o.byProduct.core.on&&o.byProduct.mini.on?'Venues '+money(o.byProduct.core.revenueYear)+' · app '+money(o.byProduct.mini.revenueYear):'Before any revenue share')+'</small></div>'+
    ...
    '<div class="tile"><span>'+(o.free?'Licence':'Licence retired by activity')+'</span><strong>'+payoff+'</strong><small>'+payoffNote+'</small></div>'+
    '<div class="tile"><span>Your prize funding / yr</span><strong>'+money(o.prizeYear)+'</strong><small>'+(o.feeYear>0?'Entries '+money(o.entriesYear)+' · head-to-head '+money(o.feeYear)+' a year':'Carried out of your share')+'</small></div>';
  // The combined model: one base, both products, three cases.
  var c=o.combined;
  h+='<h2>Combined revenue model'+(c.includeH2H&&c.includeTournaments?' · head-to-head + tournaments':c.includeH2H?' · head-to-head':' · tournaments')+'</h2><div class="hint">Averages across the '+f.term+'-year term, updated as you type. One base feeds both products. Head-to-head takes a share of it; tournaments take their own share, or the count you entered.</div>';
