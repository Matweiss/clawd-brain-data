...
function LPcalculate(input,overrides){var s=LPstate(input),errors=LPvalidate(s),term=Math.round(LPnum(s.termYears,1,3)),months=term*12,fees=s.annualFees.slice(0,term).map(function(x){return LPnum(x,0)}),totalContract=fees.reduce(function(a,b){return a+b},0),upfrontRaw=s.upfrontMode==='percent'?fees[0]*LPnum(s.upfrontValue,0,100)/100:LPnum(s.upfrontValue,0),upfront=Math.min(fees[0]||0,upfrontRaw),payC=LPnum(s.payoff.customer,0,100)/100,payL=LPnum(s.payoff.lucra,0,100)/100,creditRate=LPnum(s.payoff.credit,0,100)/100,postC=LPnum(s.post.customer,0,100)/100,postL=LPnum(s.post.lucra,0,100)/100,growthRate=LPnum(s.growthRate,0,30)/100,growthMonths=Math.round(LPnum(s.growthMonths,0,months)),baseEngagement=overrides&&overrides.engagement!==undefined?LPnum(overrides.engagement,0,100):LPnum(s.engagement,0,100),priceMultiplier=overrides&&overrides.priceMultiplier!==undefined?LPnum(overrides.priceMultiplier,0):1,useAnnualBucket=s.postMode==='year'||!s.carryForward,contractRemaining=Math.max(0,totalContract-upfront),yearRemaining=Math.max(0,(fees[0]||0)-upfront),carryBank=0,creditApplied=0,creditCapacity=0,totalCustomer=0,totalLucra=0,customerDuring=0,customerAfter=0,totalPrize=0,totalTrueUp=0,totalGross=0,largestShortfall=0,yearOneClearMonth=null,fullClearMonth=null,cumulativeActivity=upfront,rows=[],trueUps=[],warnings=[],sponsorUnused=0;
 ...
 for(var month=1;month<=months;month++){var year=Math.ceil(month/12),monthInYear=((month-1)%12)+1;if(monthInYear===1&&month>1){yearRemaining=Math.max(0,(fees[year-1]||0)-carryBank);carryBank=Math.max(0,carryBank-(fees[year-1]||0))}
  var growth=Math.pow(1+growthRate,Math.min(month-1,growthMonths)),engaged=LPnum(s.audience,0)*growth*baseEngagement/100,entryDemand=engaged*LPnum(s.rebuy,0),tournaments=s.tournaments||[],capacity=tournaments.reduce(function(a,t){return a+LPnum(t.entriesPerEvent,0)*LPnum(t.eventsPerMonth,0)},0),usedEntries=Math.min(entryDemand,capacity),tournamentGross=0,prizeCost=0,tournamentDetail=[];
  if(s.tournamentsOn&&capacity>0)tournaments.forEach(function(t){var monthlyCapacity=LPnum(t.entriesPerEvent,0)*LPnum(t.eventsPerMonth,0),entries=usedEntries*monthlyCapacity/capacity,price=LPnum(t.entryPrice,0)*priceMultiplier,gross=entries*price,prize=LPnum(t.prizeCost,0)*LPnum(t.eventsPerMonth,0);tournamentGross+=gross;prizeCost+=prize;tournamentDetail.push({name:t.name||'Tournament',entries:entries,entryPrice:price,gross:gross,prizeCost:prize,prizePct:gross>0?prize/gross:null})});
  ...
  var cashNegative=payoffGross>0&&payoffPrizeCost>payoffTournamentCustomer+0.01;if(cashNegative)warnings.push('Month '+month+': payoff-phase prize board cost exceeds the customer payoff-phase tournament share.');rows.push({month:month,year:year,growth:growth,engaged:engaged,tournamentEntries:usedEntries,tournamentGross:tournamentGross,h2hHandle:h2hHandle,h2hGross:h2hGross,miniHandle:miniHandle,miniGross:miniGross,gross:gross,licenceCredit:sponsorApplied+grossCredit,cumulativeCredit:creditApplied,balanceRemaining:contractRemaining,customerShare:customerShare,prizeCost:prizeCost,prizePct:tournamentGross>0?prizeCost/tournamentGross:null,customerNet:customerNet,lucraShare:lucraShare,trueUp:trueUp,split:split,cashNegative:cashNegative,tournaments:tournamentDetail})}
 var structureNet=totalCustomer-totalPrize-upfront-totalTrueUp,cashUpfrontNet=rows.reduce(function(a,r){return a+r.gross*postC-r.prizeCost},0)-totalContract,toggledOff=[];if(!s.tournamentsOn)toggledOff.push('tournaments');if(!s.h2hOn)toggledOff.push('head-to-head');if(!s.miniOn)toggledOff.push('mini games');if(!s.sponsorOn)toggledOff.push('sponsorship');if(LPnum(s.growthRate,0)===0)toggledOff.push('growth');return{errors:[],state:s,totalContract:totalContract,upfront:upfront,months:rows,trueUps:trueUps,totalTrueUp:totalTrueUp,creditApplied:creditApplied,creditCapacity:creditCapacity,coverage:totalContract>0?creditCapacity/totalContract:null,balanceRemaining:contractRemaining,yearOneClearMonth:yearOneClearMonth,fullClearMonth:fullClearMonth,totalGross:totalGross,totalCustomer:totalCustomer,customerDuring:customerDuring,customerAfter:customerAfter,totalLucra:totalLucra,totalPrize:totalPrize,largestMonthlyShortfall:largestShortfall,structureNet:structureNet,cashUpfrontNet:cashUpfrontNet,comparisonDelta:structureNet-cashUpfrontNet,toggledOff:toggledOff,warnings:warnings,sponsorUnused:sponsorUnused,growthRateApplied:growthRate};}
function LPbreakEvenMap(input){var s=LPstate(input),primary=(s.tournaments&&s.tournaments[0])?LPnum(s.tournaments[0].entryPrice,0):10,prices=[.5,.75,1,1.25,1.5].map(function(f){return Math.max(1,Math.round(primary*f*100)/100)}),engagements=[5,10,15,20,25],columns=prices.map(function(price){var multiplier=primary>0?price/primary:1,lo=0,hi=100,reachable=false;for(var i=0;i<14;i++){var mid=(lo+hi)/2,r=LPcalculate(s,{engagement:mid,priceMultiplier:multiplier}),clears=r.fullClearMonth!==null&&r.totalTrueUp<.01;if(clears){reachable=true;hi=mid}else lo=mid}return{price:price,required:reachable?Math.ceil(hi*10)/10:null,cells:engagements.map(function(e){var r=LPcalculate(s,{engagement:e,priceMultiplier:multiplier}),clears=r.fullClearMonth!==null&&r.totalTrueUp<.01,comfortable=clears&&r.coverage!==null&&r.coverage>=1.2;return{engagement:e,status:comfortable?'comfortable':clears?'tight':'miss',coverage:r.coverage,fullClearMonth:r.fullClearMonth,trueUp:r.totalTrueUp}})}});return{prices:prices,engagements:engagements,columns:columns}}

module.exports = { C, MGcalc, tmCompute, gmCompute, FTPcalc, FTPmatrix, FTPramp, FTP_DEFAULTS, BQcalc, BQ_DEFAULTS, DMcalc, DM_DEFAULTS, LPcalculate, LPbreakEvenMap, LPvalidate, LP_DEFAULTS };
