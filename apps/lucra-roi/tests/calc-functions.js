...
var BQ_DEFAULTS={customer:'',mode:'f2p',audience:'',locations:1,audienceBasis:'total',objective:'retention',outcomeValue:'',measurement:'linked',monthlyCost:10000,rewardCost:'',engagement:10,paidFormat:'tournament',participants:'',price:'',frequency:1,prizeCost:'',rake:20,claimRate:40,redemptionRate:30,outcomeRate:20,incrementality:50};
function BQcalc(input){var s=Object.assign({},BQ_DEFAULTS,input||{}),audience=FTPnum(s.audience,0)*(s.audienceBasis==='location'?Math.max(1,FTPnum(s.locations,1)):1),engaged=audience*FTPpct(s.engagement),f2pOn=s.mode!=='paid',paidOn=s.mode!=='f2p',f2p=f2pOn?FTPcalc({audience:audience,engagement:s.engagement,license:s.monthlyCost,implementation:0,opex:0,rewardCost:s.rewardCost,claimRate:s.claimRate,redemptionRate:s.redemptionRate,outcomeRate:s.outcomeRate,outcomeValue:s.outcomeValue,incrementality:s.incrementality,objective:s.objective,measurement:s.measurement}):null,participants=FTPnum(s.participants,0),frequency=FTPnum(s.frequency,0),price=FTPnum(s.price,0),paidGross=paidOn?(s.paidFormat==='tournament'?participants*frequency*price:participants*frequency*price*FTPpct(s.rake)):0,paidNet=paidGross-(s.paidFormat==='tournament'?FTPnum(s.prizeCost,0):0);return{state:s,audience:audience,engaged:engaged,f2p:f2p,paidGross:paidGross,paidNet:paidNet}}
